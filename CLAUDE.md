...
7. **JS interop payloads are named types, never anonymous types or positional records.** The library sets `IsAotCompatible`, so a published Blazor WebAssembly app trims its members and strips constructor parameter names. `System.Text.Json` binds a parameterized constructor by parameter name, so an anonymous type (or a `record` with a primary constructor) serializes fine in development and throws `ConstructorContainsNullParameterNames` only in the published app. Add a class with init-only properties to `Services/JsInteropPayloads.cs` and root it with `[DynamicDependency(DynamicallyAccessedMemberTypes.PublicProperties, typeof(TPayload))]` at the call site. Same hazard for components activated by `Type` at runtime — `AddOmniComponents` roots those. `PublishedRouteSweepBrowserTests` walks every showcase route against the trimmed publish and is what catches regressions here.
8. **Naming.** CSS class prefix `omni-`, CSS custom property prefix `--omni-`. JavaScript remains private to isolated ES modules and typed services. App-level CSS in consumers uses its own prefix (`fs-pdv-*`, `fs-cd-*` in FoodService).
