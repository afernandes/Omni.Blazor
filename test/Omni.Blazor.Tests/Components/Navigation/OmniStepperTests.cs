...
public class OmniStepperTests : TestContextBase
{
    ...
    {
        var cut = Render<OmniStepper>(p => p.Add(c => c.Class, "my-stepper"));
    ...
    {
        var cut = Render<OmniStepper>(p => p.Add(c => c.Style, "margin: 8px"));
    ...
    {
        var cut = Render<OmniStepper>(p => p
