...
public class OmniSwitchTests : TestContextBase
{
    ...
    {
        var cut = Render<OmniSwitch>(p => p.Add(c => c.Value, true));
    ...
    {
        var cut = Render<OmniSwitch>(p => p.Add(c => c.Label, "On"));
    ...
    {
        var cut = Render<OmniSwitch>(p => p.Add(c => c.Class, "custom-cls"));
    ...
    {
        var cut = Render<OmniSwitch>(p => p.Add(c => c.Style, "margin:4px"));
