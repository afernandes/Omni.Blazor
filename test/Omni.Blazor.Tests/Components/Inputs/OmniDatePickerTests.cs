...
public class OmniDatePickerTests : TestContextBase
{
    ...
    {
        var cut = Render<OmniDatePicker<DateOnly?>>(p => p.Add(c => c.Disabled, true));
    ...
    {
        var cut = Render<OmniDatePicker<DateOnly?>>(p => p.Add(c => c.Class, "custom-cls"));
    ...
    {
        var cut = Render<OmniDatePicker<DateOnly?>>(p => p.Add(c => c.Style, "min-width: 200px"));
