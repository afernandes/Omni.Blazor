...
public class OmniCheckBoxTests : TestContextBase
{
    ...
    {
        var cut = Render<OmniCheckBox>(p => p.Add(c => c.Value, true));
    ...
    {
        bool? captured = null;
        var cut = Render<OmniCheckBox>(p => p
            .Add(c => c.Value, false)
    ...
    {
        var cut = Render<OmniCheckBox>(p => p.Add(c => c.Label, "Aceito"));
    ...
    {
        var cut = Render<OmniCheckBox>(p => p.Add(c => c.Class, "custom-cls"));
    ...
    {
        var cut = Render<OmniCheckBox>(p => p.Add(c => c.Style, "margin:4px"));
