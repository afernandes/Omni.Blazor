...
public class OmniCardGroupTests : TestContextBase
{
    ...
    {
        var cut = Render<OmniCardGroup>(p => p.Add(c => c.Orientation, Orientation.Vertical));
        var root = cut.Find(".omni-card-group");
    ...
    {
        var cut = Render<OmniCardGroup>(p => p.Add(c => c.Responsive, false));
    ...
    {
        var cut = Render<OmniCardGroup>(p => p
