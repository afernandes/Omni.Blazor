...
public class OmniThinkingBlockTests : TestContextBase
{
    ...
    {
        var cut = RenderComponent<OmniThinkingBlock>(p => p.AddChildContent("step 1"));
    ...
    {
        var cut = RenderComponent<OmniThinkingBlock>(p => p.AddChildContent("x"));
        Assert.Contains("Reasoning", cut.Find(".omni-thinking-title").TextContent);

        cut.SetParametersAndRender(p => p.Add(c => c.Title, "Thinking").AddChildContent("x"));
    ...
    {
        var cut = RenderComponent<OmniThinkingBlock>(p => p.AddChildContent("the chain of thought"));
    ...
    {
        bool? state = null;
        var cut = RenderComponent<OmniThinkingBlock>(p => p
    ...
    {
        var cut = RenderComponent<OmniThinkingBlock>(p => p.Add(c => c.Expanded, true).AddChildContent("visible"));
    ...
    public void Recompute_does_not_fire_on_Class_change()
    {
        ...
        cut.SetParametersAndRender(p => p.Add(c => c.Class, "cc").AddChildContent("x"));
