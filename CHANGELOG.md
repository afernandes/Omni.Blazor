...
### Performance
- Markdown rendering allocates **92–94% less** and runs up to **13× faster**. `MarkdownRenderer` ran ~29 distinct patterns through the *static* `Regex` overloads, which share a process-wide cache of 15 entries — with more patterns than slots, most calls recompiled their pattern from scratch on every render, of every line. Rendering one 75-byte sentence cost 84 KB. The patterns are now `[GeneratedRegex]`, so each is a compiled static instance with no cache lookup and nothing to recompile: a 12-section document goes from 3 428 KB / 2.34 ms to 217 KB / 176 µs, and a streaming chunk from 1 715 KB / 987 µs to 109 KB / 87 µs. This matters most for `OmniStreamingText` and the AI chat components, where every chunk carries a new source and so cannot use `OmniMarkdown`'s memoisation. HTML output is unchanged, pinned by a 54-case characterization test.
- New `benchmarks/Omni.Blazor.Benchmarks` (BenchmarkDotNet) covering the CSS builders, Markdown, the DataGrid/TreeGrid hierarchy engine and the manifest generator, with the measured figures in `benchmarks/README.md` and allocation budgets asserted by the normal test suite.
